...
import addPageScript from './WMS23597.script';
import styles from './WMS23597.style';
import getVariables from './WMS23597.variables';
...
const PC_Datavalue = ({ fragment }) => {
  return (
    <WmFormField
      ...
      widget="text"
      inputtype="text"
      formRef="regFormForm1"
      primaryKey="undefined"
      isRelated="undefined"
      onChange={($event, widget, newVal, oldVal) => {
        fragment.Widgets.regFormForm1 &&
          fragment.Widgets.regFormForm1.props.onChange &&
          fragment.Widgets.regFormForm1.props.onChange(
            $event,
            widget,
            newVal,
      ...
      renderFormFields={$formField => (
        <>
          ...
          <WmText
            ...
            classname="form-input form-text form-dataValue-input"
            listener={fragment}
          ></WmText>
        </>
      )}
      ...
      renderFormFields={$formField => (
        <>
          <WmLabel
            ...
            memoize="false"
            required={$formField.required}
            name="password_formLabel"
            classname="form-label password_formLabel"
            listener={fragment}
          ></WmLabel>
          <WmText
            name="password"
            ...
            updateon={$formField.updateon || 'blur'}
            maxchars={fragment.toNumber($formField.maxchars)}
            classname="form-input form-text form-password-input"
            listener={fragment}
          ></WmText>
        </>
      )}
...
const PC_Confirmpassword = ({ fragment }) => {
  return (
    <WmFormField
      ...
      renderFormFields={$formField => (
        <>
          <WmLabel
            ...
            memoize="false"
            required={$formField.required}
            name="confirmpassword_formLabel"
            classname="form-label confirmpassword_formLabel"
            listener={fragment}
          ></WmLabel>
          <WmText
            name="confirmpassword"
            ...
            updateon={$formField.updateon || 'blur'}
            maxchars={fragment.toNumber($formField.maxchars)}
            classname="form-input form-text form-confirmpassword-input"
            listener={fragment}
          ></WmText>
        </>
      )}
  ...
  return (
    ...
    >
      <WmForm
        ...
        method="post"
        dataset={fragment.Variables.regForm.dataSet}
        captionalign="left"
        name="regFormForm1"
        listener={fragment}
        onChange={($event, widget, newVal, oldVal) => {
          if (fragment.Variables.regForm?.twoWayBinding && newVal !== oldVal) {
            fragment.Variables.regForm.dataSet[widget.props.name] = newVal;
          }
        }}
        formSubmit={(formData, success, error) => {
          fragment.Variables.regForm &&
            fragment.Variables.regForm.invoke(formData, success, error);
        }}
      >
        <WmFormBody name="wm_form_body_76faajbgia" listener={fragment}>
          <WmLayoutgrid columns="1" name="layoutgrid1" listener={fragment}>
            ...
            <WmGridrow name="gridrow2" listener={fragment}>
              <WmGridcolumn
                ...
                listener={fragment}
              >
                <PC_Confirmpassword fragment={fragment} />
              </WmGridcolumn>
            </WmGridrow>
          </WmLayoutgrid>
        </WmFormBody>
        <WmFormFooter name="wm_form_footer_779fg1h4a2" listener={fragment}>
          <WmFormAction
            show={true}
            ...
            disabled={false}
            widget-type="button"
            formKey="regFormForm1"
            name="regFormForm1_reset_formAction"
            displayName="Reset"
            updateMode={true}
            action="()=&gt; fragment.Widgets.regFormForm1.formreset()"
            btnClass="btn-default"
            classname="form-reset btn-default"
            listener={fragment}
            formAction={$event => {
              fragment.Widgets.regFormForm1.formreset();
            }}
          ></WmFormAction>
          <WmFormAction
            show={true}
            ...
            widget-type="button"
            formKey="regFormForm1"
            name="regFormForm1_submit_formAction"
            displayName="Save"
            updateMode={true}
            action="()=&gt; fragment.Widgets.regFormForm1.submit()"
            btnClass="btn-primary"
            classname="form-save btn-success"
            listener={fragment}
            formAction={$event => {
              fragment.Widgets.regFormForm1.submit();
            }}
          ></WmFormAction>
        </WmFormFooter>
      </WmForm>
...
export default class WMS23597Page extends BasePage {
  components;
  ...
  constructor(props) {
    ...
    this.theme = this.theme.$new('WMS23597-styles', styles);
    if (styleOverrides) {
      this.theme = this.theme.$new('WMS23597-styleOverrides', styleOverrides);
    }
  }
